...
class LiftLemon(SingleArmEnv):
    """
    This class corresponds to the lemon lifting task for a single robot arm.
    ...
    Args:
        ...
        use_object_obs (bool): if True, include object (lemon) information in
            the observation.
    ...
    def reward(self, action=None):
        ...
        # use a shaping reward
            ...
            dist = np.linalg.norm(gripper_site_pos - lemon_pos)
            reaching_reward = 1 - np.tanh(10.0 * dist)
            reward += reaching_reward

            # grasping reward
            if self._check_grasp(gripper=self.robots[0].gripper, object_geoms=self.lemon):
                reward += 0.25
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            @sensor(modality=modality)
            def gripper_to_lemon_pos(obs_cache):
                return (
                    obs_cache[f"{pf}eef_pos"] - obs_cache["lemon_pos"]
                    if f"{pf}eef_pos" in obs_cache and "lemon_pos" in obs_cache
                    else np.zeros(3)
                )

            sensors = [lemon_pos, lemon_quat, gripper_to_lemon_pos]
